...
public class RestoreToDirectory : IRestoreVersionMethod
{
    public void Execute(RestorePoint restorePoint, ICompressionMetod method, IRepository repository, string dirPath = null)
    {
        if (string.IsNullOrWhiteSpace(dirPath)) throw new ArgumentNullException(nameof(dirPath));
        switch (method)
        {
            case SplitStorage:
                ...
                {
                    File.Move($"{restorePoint.FullPointPath}\\{o.Name}", $"{Path.Combine(dirPath, o.Name)}", true);
                }
